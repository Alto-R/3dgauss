import { Coroutine, createYieldingScheduler, runCoroutineAsync } from './coroutine';
...
export class GaussianSplattingSorter {
    ...
    onmessage: ((this: GaussianSplattingSorter, ev: Uint32Array) => void) | null = null;
    ...
    private *_sortData(viewProj: number[], isAsync: boolean): Coroutine<void> {
        if (!this.hasInit) {
            this._initSortData();
        }
    ...
    public async sortDataAsync(viewProj: number[]): Promise<void> {
