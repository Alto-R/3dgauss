...
/** @internal */
export type CoroutineScheduler<T> = (coroutine: AsyncCoroutine<T>, onStep: (stepResult: CoroutineStep<T>) => void, onError: (stepError: unknown) => void) => void;
...
export function inlineScheduler<T>(coroutine: AsyncCoroutine<T>, onStep: (stepResult: CoroutineStep<T>) => void, onError: (stepError: unknown) => void) {
    try {
...
export function createYieldingScheduler<T>(yieldAfterMS = 25) {
    ...
    return (coroutine: AsyncCoroutine<T>, onStep: (stepResult: CoroutineStep<T>) => void, onError: (stepError: unknown) => void) => {
 ...
 */
// eslint-disable-next-line @typescript-eslint/naming-convention
export function runCoroutine<T>(coroutine: AsyncCoroutine<T>, scheduler: CoroutineScheduler<T>, onSuccess: (result: T) => void, onError: (error: unknown) => void, abortSignal?: AbortSignal) {
    const resume = () => {
