...
export class GLTFGaussianSplattingExtension implements GLTFLoaderPlugin {
    ...
    loadMesh(meshIndex: number): Promise<Group | Mesh> | null {
        ...
        const pending: Promise<BufferGeometry[]>[] = [];
        ...
        return Promise.all(pending).then(function (results) {
            ...
            const pending: Promise<GaussianSplattingMesh>[] = [];

            function loadGaussianSplattingMesh(geometry: BufferGeometry) {
            ...
            for (let i = 0, il = geometries.length; i < il; i++) {
                ...
                pending.push(loadGaussianSplattingMesh(geometry));
            }
     ...
     */
    loadGeometries(primitives: Array<unknown>): Promise<Array<BufferGeometry>> {
        const parser = this.parser;
        ...
        for (let i = 0, il = primitives.length; i < il; i++) {
            ...
            const geometryPromise = this.addPrimitiveAttributes(new BufferGeometry(), primitive, parser);
     ...
     * @param {unknown} primitiveDef
     * @param {GLTFParser} parser
     * @return {Promise<BufferGeometry>}
     */
    addPrimitiveAttributes(geometry: BufferGeometry, primitiveDef: any, parser: GLTFParser): Promise<BufferGeometry> {
 ...
 * @param {GLTFParser} parser
 */
function computeBounds(geometry: BufferGeometry, primitiveDef: any, parser: GLTFParser) {
    const attributes = primitiveDef.attributes;
...
function getNormalizedComponentScale(constructor: typeof Int8Array | typeof Uint8Array | typeof Int16Array | typeof Uint16Array | typeof Uint32Array | typeof Float32Array) {
    // Reference:
